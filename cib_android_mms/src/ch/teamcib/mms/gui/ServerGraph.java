...
/**
 * The ServerGraph activity initializes the RAM-Graph
 * 
 * @author CiB
 *
 */
...
//			Toast.makeText(this, cur.getString(3).split("/")[0], Toast.LENGTH_SHORT).show();
			if (cur.getString(3).equals("~")){
				...
				maxGB = (int) Math.ceil(Float.valueOf(cur.getString(3).split("/")[1]).floatValue() / 1000);
			}
		...
		GraphView graphView = new GraphView(this, vals, "RAM workload", 
				horlabels, maxGB, GraphView.LINE);
